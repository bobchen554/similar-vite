...
function handleHTML(ctx) {
  ctx.type = "text/html"
  ctx.body = `${readFileSync('./index.html')}<script>window.process = {env: {NODE_ENV: "development"}}</script>`
}

function handleJS(ctx) {
  const { url } = ctx.request
  ctx.type = "application/javascript"
  ctx.body = rewriteImport(readFileSync(url.slice(1)))
}

function handleModule(ctx) {
  const { url } = ctx.request
  ctx.type = "application/javascript"
